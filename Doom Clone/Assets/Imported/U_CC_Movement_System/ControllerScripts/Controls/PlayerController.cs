...
public class PlayerController : MonoBehaviour
{
    ...
    [SerializeField] private float mouselookClamp;
    ...
    {
        Vector2 targetMouseDelta = (mouseLookInput * Time.fixedDeltaTime);

        currentMouseDelta = Vector2.SmoothDamp(currentMouseDelta, targetMouseDelta, ref currentMouseDeltaVelocity, mouseSmoothTime *  Time.deltaTime);

        cameraPitch -= currentMouseDelta.y * mouseSensitivity;
        cameraPitch = Mathf.Clamp(cameraPitch, -mouselookClamp, mouselookClamp);
    ...
    void UpdateRunning()
    {
        if (!alwaysRun && running == 1 && canRun)
        {
            SetRun();
        }
        else if(alwaysRun && running == 0 && canRun){
            SetRun();
        }
        else
        {
